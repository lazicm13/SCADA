...
namespace ProcessingModule
{
    ...
    public class ProcessingManager : IProcessingManager
    {
        ...
        public void ExecuteWriteCommand(IConfigItem configItem, ushort transactionId, byte remoteUnitAddress, ushort pointAddress, int value)
        {
            ...
            List<PointIdentifier> list = new List<PointIdentifier> {ventil, pumpa1, pumpa2, pumpa3 };
            List<IPoint> points = storage.GetPoints(list);

            if((points[1].RawValue == 1 || points[2].RawValue == 1 || points[3].RawValue == 1) && (pointAddress == 2000 && value == 0))
            {
                return;
            }

            if(points[0].RawValue == 0 && (pointAddress == 3000 || pointAddress == 3001 || pointAddress == 3002))
            {
                return;
            }
